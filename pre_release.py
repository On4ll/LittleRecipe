...
def beam_search(recipe, priority_stats, banned_ingredients, must_have_ingredients, top_x=5, progress_callback=None):
    beam_width = 1000 if len(recipe) > 2 else 100000
    slots = []
    ...
    for combo, stats in beam:
        ...
        if all(stat_dict[stat] > 0 for stat in priority_stats):
            ...
            if must_have_ingredients:
                ...
                #combo_counts = {must: combo_ingredients.count(must) for must in set(must_have_ingredients)}
...
class RecipeApp(ctk.CTk):
    def __init__(self):
        ...
        #self.priority_label = ctk.CTkLabel(self.priority_frame, text="Priority Stats:", font=("Arial", 20))
        ...
        # Progress Bar
        self.progress_bar = ctk.CTkProgressBar(self.input_frame, width=300)
        ...
        self.banlist_scroll_frame = ctk.CTkScrollableFrame(self.banlist_display_frame, width=800, height=130)
    ...
    def calculate_recipes(self):
        ...
        result_window.lift()  # Bring the window to the front
